...
import { JWT_EXPIRES_IN, signToken, verifyToken  } from "../core/auth/auth.utils";
...
export class UserController {
  ...
  async login(req: Request, res: Response) {
    ...
    // Tạo access_token
    const access_token = signToken({ id: user.id, email: user.email, username: user.username });
    // Nếu muốn, có thể tạo refresh_token riêng
    const refresh_token = signToken({ id: user.id, type: "refresh" });
  ...
  const users = await this.service.search(req.query as any);
